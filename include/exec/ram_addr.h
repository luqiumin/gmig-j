...
static inline void gpu_physical_memory_set_dirty_range(ram_addr_t start,
                                                       ram_addr_t length)
{
    unsigned long end, page;
...
// static inline void gpu_physical_memory_set_refresh_range(ram_addr_t start,
//                                                        ram_addr_t length)
// {
//     unsigned long end, page;
...
//     end = TARGET_PAGE_ALIGN(start + length) >> TARGET_PAGE_BITS;
//     page = start >> TARGET_PAGE_BITS;
//     bitmap_set(ram_list.dirty_memory[DIRTY_MEMORY_VGPU_REFRESH], page, end - page);
//     // xen_modified_memory(start, length);
// }
...
static inline void gpu_physical_memory_set_dirty_lebitmap(unsigned long *bitmap,
                                                          ram_addr_t start,
                                                          ram_addr_t pages)
{
    unsigned long i, j;
        ...
        for (i = 0; i < len; i++) {
            if (bitmap[i] != 0) {
                ...
                do {
                    ...
                    addr = page_number * TARGET_PAGE_SIZE;
                    ram_addr = start + addr;
                    gpu_physical_memory_set_dirty_range(ram_addr,
                                       TARGET_PAGE_SIZE * hpratio);
                } while (c != 0);
            }
        }
...
// static inline void gpu_physical_memory_set_refresh_lebitmap(unsigned long *bitmap,
//                                                           ram_addr_t start,
//                                                           ram_addr_t pages)
// {
//     unsigned long i, j;
...
//                 ram_list.dirty_memory[DIRTY_MEMORY_VGPU_REFRESH][page + k] |= temp;
//             }
//         }
//         // xen_modified_memory(start, pages);
//     } else {
...
//                     addr = page_number * TARGET_PAGE_SIZE;
//                     ram_addr = start + addr;
//                     gpu_physical_memory_set_refresh_range(ram_addr,
//                                        TARGET_PAGE_SIZE * hpratio);
//                 } while (c != 0);
//             }
//         }
